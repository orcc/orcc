...
extern struct fifo_s *address_01_MV;
extern struct fifo_s *address_01_BTYPE;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs

extern struct fifo_s *address_01_RA;
extern struct fifo_s *address_01_WA;
extern struct fifo_s *address_01_halfpel;
...
static void cmd_newVop() {
  ...
  BTYPE = getReadPtr(address_01_BTYPE, 1);
  cmd_1 = BTYPE[0];
  round = (cmd_1 & 32) != 0;
}
...
static int isSchedulable_cmd_newVop() {
  ...
  _tmp1_1 = hasTokens(address_01_BTYPE, 1);
  if (_tmp1_1) {
    BTYPE = getPeekPtr(address_01_BTYPE, 1);
    cmd_1 = BTYPE[0];
...
static int isSchedulable_getmvx() {
  ...
  _tmp1_1 = hasTokens(address_01_MV, 1);
  if (_tmp1_1) {
    _tmp0_1 = 1;
    _tmp0_2 = _tmp0_1;
  } else {
...
static void getmvy() {
  ...
  halfpel = getWritePtr(address_01_halfpel, 1);
  MV = getReadPtr(address_01_MV, 1);
  y_1 = MV[0];
  interp_y0_1 = (y_1 & 1) == 1;
  mvy = y_1 >> 1;
...
static void write_addr() {
  ...
  WA = getWritePtr(address_01_WA, 1);
  _tmp0_1 = write_count;
  _tmp1_1 = write_count;
...
static void read_addr() {
  ...
  RA = getWritePtr(address_01_RA, 1);
  _tmp0_1 = mvx;
  _tmp1_1 = read_col;
  _tmp2_1 = mvy;
  _tmp3_1 = read_row;
    ...
    if (isSchedulable_cmd_noMotion()) {
      ...
      if (isSchedulable_cmd_motion()) {
        ...
        if (isSchedulable_cmd_neither()) {
          if (hasRoom(address_01_halfpel, 1)) {
            cmd_neither();
            _FSM_state = s_init;
            res = 1;
          } else {
