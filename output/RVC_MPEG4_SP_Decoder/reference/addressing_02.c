...
extern struct fifo_s *addressing_02_BTYPE;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs

extern struct fifo_s *addressing_02_A;
extern struct fifo_s *addressing_02_B;
extern struct fifo_s *addressing_02_C;
...
static void start() {
  ...
  BTYPE = getReadPtr(addressing_02_BTYPE, 1);
  mbx = 0;
  top_edge = 1;
  left_edge = 1;
  comp = 0;
...
static int isSchedulable_start() {
  ...
  _tmp1_1 = hasTokens(addressing_02_BTYPE, 1);
  if (_tmp1_1) {
    BTYPE = getPeekPtr(addressing_02_BTYPE, 1);
    cmd_1 = BTYPE[0];
...
static int isSchedulable_read_intra() {
  ...
  if (_tmp1_1) {
    BTYPE = getPeekPtr(addressing_02_BTYPE, 1);
    type_1 = BTYPE[0];
    _tmp0_1 = (type_1 & 1024) != 0 && (type_1 & 2048) == 0;
    _tmp0_2 = _tmp0_1;
  } else {
    _tmp0_3 = 0;
    _tmp0_2 = _tmp0_3;
...
static void predict_b0() {
  ...
  A = getWritePtr(addressing_02_A, 1);
  B = getWritePtr(addressing_02_B, 1);
  C = getWritePtr(addressing_02_C, 1);
  a0_1 = 0;
  b0_1 = 0;
  c0_1 = 0;
  _tmp0_1 = left_edge;
...
static int predict_state_scheduler() {
  ...
  if (isSchedulable_predict_b2()) {
    if (hasRoom(addressing_02_A, 1) && hasRoom(addressing_02_B, 1) && hasRoom(addressing_02_C, 1)) {
      predict_b2();
