...
extern struct fifo_s *addressing_01_BTYPE;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs

extern struct fifo_s *addressing_01_A;
extern struct fifo_s *addressing_01_B;
extern struct fifo_s *addressing_01_C;
...
static void start() {
  ...
  BTYPE = getReadPtr(addressing_01_BTYPE, 1);
  mbx = 0;
  top_edge = 1;
  left_edge = 1;
}
...
static int isSchedulable_start() {
  ...
  _tmp1_1 = hasTokens(addressing_01_BTYPE, 1);
  if (_tmp1_1) {
    BTYPE = getPeekPtr(addressing_01_BTYPE, 1);
    cmd_1 = BTYPE[0];
...
static void predict_b0() {
  ...
  A = getWritePtr(addressing_01_A, 1);
  B = getWritePtr(addressing_01_B, 1);
  C = getWritePtr(addressing_01_C, 1);
  a0_1 = 0;
  b0_1 = 0;
  c0_1 = 0;
  _tmp0_1 = left_edge;
...
static int predict_state_scheduler() {
  ...
  if (isSchedulable_predict_b0()) {
    if (hasRoom(addressing_01_A, 1) && hasRoom(addressing_01_B, 1) && hasRoom(addressing_01_C, 1)) {
      predict_b0();
