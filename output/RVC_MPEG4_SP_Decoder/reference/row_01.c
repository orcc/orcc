...
extern struct fifo_s *row_01_X;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs

extern struct fifo_s *row_01_Y;
...
static void untagged01() {
  ...
  Y = getWritePtr(row_01_Y, 8);
  X = getReadPtr(row_01_X, 8);
  _i0_1 = 0;
  _i0_3 = _i0_1;
  while (_i0_3 < 8) {
    _tmp0_1 = X[1 * _i0_3 + 0];
...
int row_01_scheduler() {
  int res = 1;
  
  while (res) {
    if (isSchedulable_untagged01()) {
      if (hasRoom(row_01_Y, 8)) {
        untagged01();
        res = 1;
      } else {
        res = 0;
