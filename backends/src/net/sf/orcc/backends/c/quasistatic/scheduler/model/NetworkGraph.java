...
public class NetworkGraph {
	 ...
	 */
	public Graph<Action, DefaultEdge> createSystemLevelGraph()
			throws OrccException {
		// TODO: create a system-level graph once all the actors have been
		// unrolled
		return new DirectedMultigraph<Action, DefaultEdge>(DefaultEdge.class);
		/*
		 * Graph graph = new DirectedMultigraph<Action,
		 * DefaultEdge>(DefaultEdge.class); DSEInputFilesCreator
		 * dseInputFilesCreator = new DSEInputFilesCreator();
		 * 
		 * //Inserts the actions/connections of each actor for(ActorGraph actor:
		 * scheduledActorsList){ //Take the actor subgraph Graph<Action,
		 * DefaultEdge> subgraph = actor.getGraph(Switch.getBTYPE()); subgraph.
		 * graph.addEdges(subgraph.edgeSet());
		 * graph.addVertices(subgraph.vertexSet()); }
		 * 
		 * //Inserts the connections between network's actors for(Connection
		 * connection: getNetworkConnections()){ //Looks for the source and
		 * target actors Vertex srcVertex =
		 * network.getGraph().getEdgeSource(connection); Vertex tgtVertex =
		 * network.getGraph().getEdgeTarget(connection); Port srcPort =
		 * connection.getSource(); Port tgtPort = connection.getTarget();
		 * if(srcVertex.isInstance() && tgtVertex.isInstance()){ Instance
		 * srcInstance = srcVertex.getInstance(); Instance tgtInstance =
		 * tgtVertex.getInstance(); if(srcInstance.isActor() &&
		 * tgtInstance.isActor()){ ActorGraph srcActor =
		 * getActorGraph(srcInstance.getActor()); ActorGraph tgtActor =
		 * getActorGraph(tgtInstance.getActor()); if(srcActor != null &&
		 * tgtActor != null){ //Looks for the implicated actions List<Action>
		 * srcActions = srcActor.getActionsWithPort(srcPort); List<Action>
		 * tgtActions = tgtActor.getActionsWithPort(tgtPort); List<GraphEdge>
		 * edges = new ArrayList<GraphEdge>(); if(srcActions.size() ==
		 * tgtActions.size()){ for(int i = 0 ; i < srcActions.size() ; i ++){
		 * GraphVertex srcGraphVertex = graph.getVertex(srcActions.get(i));
		 * GraphVertex tgtGraphVertex = graph.getVertex(tgtActions.get(i));
		 * if(srcGraphVertex != null && tgtGraphVertex != null){ edges.add(new
		 * GraphEdge(srcGraphVertex, tgtGraphVertex)); } } } else{
		 * loop1:for(Action srcAction:srcActions){ GraphVertex srcGraphVertex =
		 * graph.getVertex(srcAction); if(srcGraphVertex == null){ continue
		 * loop1; } loop2:for(Action tgtAction: tgtActions){ GraphVertex
		 * tgtGraphVertex = graph.getVertex(tgtAction); if(tgtGraphVertex ==
		 * null){ continue loop2; } edges.add(new GraphEdge(srcGraphVertex,
		 * tgtGraphVertex)); } } }
		 * dseInputFilesCreator.addConnections(srcActor.getName(),
		 * tgtActor.getName(), edges);
		 * 
		 * } } } }
		 * 
		 * //Registers the new graph graphsMap.put(Switch.getBTYPE(), graph);
		 * //Prints DSE scheduler input files dseInputFilesCreator.print();
		 * 
		 * return graph;
		 */
	}
