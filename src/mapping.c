...
//    loadNetwork("/home/asanchez/tools/metis-example.xdf", network);
    load_network("/home/asanchez/tools/metis-directed-example.xdf", network);

    mapping_t *mapping = allocate_mapping(opt->nb_processors);

    do_mapping(network, *opt, mapping);

    save_mapping("/home/asanchez/tools/partition.xml", mapping);
