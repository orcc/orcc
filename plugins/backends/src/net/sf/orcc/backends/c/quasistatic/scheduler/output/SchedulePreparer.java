...
public class SchedulePreparer {
	...
	private static void copyInputData() throws QuasiStaticSchedulerException {
		...
		if (!srcFile.exists()) {
			throw new QuasiStaticSchedulerException("The file "
					+ Constants.INPUT_FILE_NAME + " was not found at "
					+ sourceFilesPath);
		}
		File dstFile = new File(Scheduler.workingDirectoryPath + File.separator
				+ Constants.INPUT_FILE_NAME);
		try {
	...
	private static void createOutputDirectories() {
		DSEScheduler.INPUT_FOLDER = Scheduler.workingDirectoryPath
				+ File.separator + Constants.DSE_INPUT_PATH + File.separator;
		DSEScheduler.OUTPUT_FOLDER = Scheduler.workingDirectoryPath
				+ File.separator + Constants.SCHEDULE_FILES_PATH
				+ File.separator;
	...
	public static boolean needsToPrepare() {
		return !(new File(DSEScheduler.INPUT_FOLDER).exists()
				&& new File(DSEScheduler.OUTPUT_FOLDER).exists() && new File(
				Scheduler.workingDirectoryPath + File.separator
						+ Constants.INPUT_FILE_NAME).exists());
	}
