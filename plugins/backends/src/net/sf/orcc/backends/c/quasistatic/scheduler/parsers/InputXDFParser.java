...
public class InputXDFParser {
	...
	public HashMap<String, String> parseCustomIndividualBuffersSizes()
			throws OrccException {
	...
	private TokensPattern parsePortsPattern(Node actorNode) {
		...
		for (int i = 0; i < portsNodes.getLength(); i++) {
			...
			if (portNode.getNodeName().equals("Port")) {
				...
				int noReads = Integer.parseInt(portElement
						.getAttribute("reads"));
				int consumptionTokens = Integer.parseInt(portElement
						.getAttribute("consumption"));
	...
	public HashMap<String, List<TokensPattern>> parseTokensPattern()
			throws OrccException {
		NodeList btypesNodes = parseGroupNodes(Constants.TOKENS_PATTERN);
