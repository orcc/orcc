...
extern struct fifo_s *invpred_BTYPE;
extern struct fifo_s *invpred_A;
extern struct fifo_s *invpred_B;
extern struct fifo_s *invpred_C;
extern struct fifo_s *invpred_QFS_DC;

////////////////////////////////////////////////////////////////////////////////
// Output FIFOs

extern struct fifo_s *invpred_QF_DC;
extern struct fifo_s *invpred_PTR;
extern struct fifo_s *invpred_AC_PRED_DIR;
extern struct fifo_s *invpred_SIGNED;
extern struct fifo_s *invpred_QUANT;
...
static void start() {
  ...
  AC_PRED_DIR = getWritePtr(invpred_AC_PRED_DIR, 1);
  BTYPE = getReadPtr(invpred_BTYPE, 1);
  cmd_1 = BTYPE[0];
  ptr = 1;
  QP = cmd_1 & 31;
  _tmp1_1 = QP;
...
static int isSchedulable_start() {
  ...
  _tmp1_1 = hasTokens(invpred_BTYPE, 1);
  if (_tmp1_1) {
    BTYPE = getPeekPtr(invpred_BTYPE, 1);
    cmd_1 = BTYPE[0];
...
static void read_inter_ac() {
  ...
  AC_PRED_DIR = getWritePtr(invpred_AC_PRED_DIR, 1);
  PTR = getWritePtr(invpred_PTR, 1);
  SIGNED = getWritePtr(invpred_SIGNED, 1);
  QUANT = getWritePtr(invpred_QUANT, 1);
  BTYPE = getReadPtr(invpred_BTYPE, 1);
  is_signed = 1;
  AC_PRED_DIR[0] = 0;
  PTR[0] = 0;
...
static void read_intra() {
  ...
  BTYPE = getReadPtr(invpred_BTYPE, 1);
  cmd_1 = BTYPE[0];
  A = getReadPtr(invpred_A, 1);
  a_1 = A[0];
  B = getReadPtr(invpred_B, 1);
  b_1 = B[0];
  C = getReadPtr(invpred_C, 1);
  c_1 = C[0];
  _tmp0_1 = dc_buf[a_1];
  dca0_1 = _tmp0_1;
  _tmp1_1 = dc_buf[b_1];
...
static int isSchedulable_read_intra() {
  ...
  _tmp1_1 = hasTokens(invpred_BTYPE, 1);
  _tmp2_1 = hasTokens(invpred_A, 1);
  _tmp3_1 = hasTokens(invpred_B, 1);
  _tmp4_1 = hasTokens(invpred_C, 1);
...
static void getdc_inter() {
  ...
  QFS_DC = getReadPtr(invpred_QFS_DC, 1);
  dc_1 = QFS_DC[0];
  _tmp0_1 = QP;
  _tmp1_1 = abs_(dc_1);
  _tmp2_1 = round;
...
static int isSchedulable_getdc_inter() {
  ...
  _tmp1_1 = hasTokens(invpred_QFS_DC, 1);
  if (_tmp1_1) {
    _tmp0_1 = 1;
    _tmp0_2 = _tmp0_1;
  } else {
...
static void sat() {
  ...
  QF_DC = getWritePtr(invpred_QF_DC, 1);
  _tmp0_1 = dc_val;
...
static int read_state_scheduler() {
  ...
  if (isSchedulable_start()) {
    if (hasRoom(invpred_AC_PRED_DIR, 1)) {
      start();
      _FSM_state = s_getw;
      res = 1;
    } else {
    ...
    if (isSchedulable_read_inter_ac()) {
      if (hasRoom(invpred_AC_PRED_DIR, 1) && hasRoom(invpred_PTR, 1) && hasRoom(invpred_QUANT, 1) && hasRoom(invpred_SIGNED, 1)) {
        read_inter_ac();
        _FSM_state = s_inter;
        res = 1;
      } else {
